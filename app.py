...
VERSION = "QS-2025-09-18-v14-optimized"
...
# ---------- Unicode font helpers (met fallback) ----------
FONTS_READY = False
FONT_REGULAR = "DejaVu"
FONT_BOLD = "DejaVuB"
...
def ensure_unicode_fonts():
    """Download DejaVu fonts als ze nog niet aanwezig zijn."""
    global FONTS_READY
    if FONTS_READY:
        return True
...
def sanitize_text_for_latin1(txt: str) -> str:
    """Maak tekst PDF-veilig als Unicode niet lukt."""
...
# ---------- Logo ----------
DEFAULT_LOGO_URL = os.getenv("LOGO_URL", "https://www.veerenstael.nl/wp-content/uploads/2020/06/logo-veerenstael-wit.png")
...
# ---------- Model-afbeelding (voor stoplichten) ----------
...
class ReportPDF(FPDF):
    ...
    def row_two_cols(self, left, right, cust):
        ...
        # onderband met alleen klantcijfer
        yb = y0 + h - band_h
...
def build_stoplight_overlay(section_labels, section_scores, out_path="stoplicht.png"):
    ...
    fig = plt.figure(figsize=(w / 100, h / 100), dpi=75)
    ax = plt.axes([0, 0, 1, 1])
    ax.imshow(img)
    ax.axis("off")
    ...
    fig.savefig(out_path, dpi=75, transparent=False, optimize=True)
    plt.close(fig)
    plt.clf()
    gc.collect()
    return out_path
...
def submit():
    ...
    try:
        ...
        for sect, rows in grouped.items():
            ...
            # gemiddelde per onderwerp voor radar/stoplicht (alleen klant)
        ...
        # Radar + Stoplichtpagina's (alleen als er onderwerpen zijn)
        if radar_sections:
            # Radar pagina
            pdf.add_page()
        ...
        if email_user and email_pass and email_to:
            try:
                ...
                body = (
                    ...
                    "Achterin vindt u een radargrafiek en het stoplichtoverzicht per onderwerp.\n\n"
        ...
        return jsonify({"email_sent": email_sent}), 200
